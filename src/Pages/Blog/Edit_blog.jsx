...
import { useAddBlogMutation, useAddCategoryMutation, useEditBlogMutation } from "../../services/apiSlice";
...
const Create_category = () => {
    ...
    const handlesubmit = async () => {
        ...
        try {
            ...
            if (image) {
                formData.append("blog_picture", image); // should be File object
            }
            ...
            const response = await editblog(formData).unwrap();
    ...
    useEffect(() => {
        if (post) {
            ...
            setForm({
                ...
                body: post?.body, // from Jodit
                blog_picture: import.meta.env.VITE_API_BASE_URL + post?.blog_picture, // file
            })


            setStatusActive(post?.publish_blog);
            setStatusInactive(!post?.publish_blog);
            setdraft_active(post?.draft_mode);
            setdraft_inactive(!post?.draft_mode);
            setreviewed_active(post?.reviewed);
            setreviewed_inactive(!post?.reviewed);
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    <SubHeader pageName={"Blog Edit"} />
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block">
                        <h3 className="text-[22px] font-[700] text-[#707793] uppercase">Blog Edit</h3>
                        <div className="flex justify-end items-center max-lg:justify-start">
                            <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb">
                                <ol className="flex flex-wrap items-center">
                                    ...
                                    <li className="text-[var(--blue)] font-medium text-[12px]">Edit Blog</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                                    ...
                                    <>
                                        ...
                                        {!form?.blog_picture ? (
                                            ...
                                            >
                                                <img
                                                    src={
                                                        typeof form.blog_picture === "string"
                                                            ? form.blog_picture // API URL
                                                            : URL.createObjectURL(form.blog_picture) // Local uploaded file
                                                    }
