...
import { useGetOrder_allQuery, useGetOrderQuery, useGetProfileQuery } from "../../services/apiSlice";
...
const Order = () => {
    ...
    const { data: order } = useGetOrder_allQuery({
        size: itemsPerPage,
        page: currentPage,
    })
    ...
    const formatDate = (isoDate) => {
        ...
        const month = String(date.getMonth() + 1).padStart(2, '0'); // e.g., 05
    ...
    const totalPages = order?.total_pages
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    ...
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        ...
                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">
                                ...
                                <li className="text-primary font-medium text-[12px]">All Orders</li>
                            </ol>
                        </nav>
                    </div>
                    <div className="grid grid-cols-4 gap-4 max-xl:grid-cols-2 max-sm:grid-cols-1 max-sm:gap-2">
                        <div className="border  bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F] p-[20px] rounded-[10px] flex justify-between">
                            <div>
                                <h4 className="mb-2 text-[#313b5e] text-[16px] font-[600]">Payment Refund</h4>
                                <p className="text-[22px] text-[#5d7186]">490</p>
                            </div>
                            <div className="bg-[#ff6c2f1a] text-primary h-[56px] w-[56px] rounded-[8px] flex justify-center items-center"><svg xmlns="http://www.w3.org/2000/svg" width="1.8em" height="1.8em" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"><path d="M12 15.333c1.105 0 2-.746 2-1.666S13.105 12 12 12s-2-.746-2-1.667c0-.92.895-1.666 2-1.666m0 6.666c-1.105 0-2-.746-2-1.666m2 1.666V16m0-8v.667m0 0c1.105 0 2 .746 2 1.666"></path><path d="M17 3.338A9.95 9.95 0 0 0 12 2C6.477 2 2 6.477 2 12c0 1.6.376 3.112 1.043 4.453c.178.356.237.763.134 1.148l-.595 2.226a1.3 1.3 0 0 0 1.591 1.592l2.226-.596a1.63 1.63 0 0 1 1.149.133A9.96 9.96 0 0 0 12 22c5.523 0 10-4.477 10-10c0-1.821-.487-3.53-1.338-5"></path></g></svg></div>
                        </div>
                        <div className="border  bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F] p-[20px] rounded-[10px] flex justify-between">
                            <div>
                                <h4 className="mb-2 text-[#313b5e] text-[16px] font-[600]">Payment Refund</h4>
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        ...
                        {/* <div>
                            <button className="text-[#5d7186] border-[1.5px] border-[#5d7186] px-2 py-1 rounded-[6px] font-[600] text-[12px]">Draft</button>
                            <button className="text-[#f9b931] border-[1.5px] border-[#f9b931] px-2 py-1 rounded-[6px] font-[600] text-[12px]">Packaging</button>
                            <button className="text-[#22c55e] border-[1.5px] border-[#22c55e] px-2 py-1 rounded-[6px] font-[600] text-[12px]">Completed</button>
                            <button className="text-[#ef5f5f] border-[1.5px] border-[#ef5f5f] px-2 py-1 rounded-[6px] font-[600] text-[12px]">Canceled</button>


                            <button className="  bg-[#eef2f7] px-2 py-1 rounded-[6px] font-[550] text-[12px]">Unpaid</button>
                            <button className=" text-white bg-[#22c55e] px-2 py-1 rounded-[6px] font-[550] text-[12px]">Paid</button>
                        </div> */}
                            ...
                            <table className="w-full ">
                                <thead className="border-y border-[#ddd]">
                                    ...
                                    <tr className="max-sm:h-[40px] h-[54px] ">
                                        ...
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-md:hidden max-md:text-[13px] max-sm:px-[10px]">Total Price</th>
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-md:text-[13px] max-sm:hidden max-sm:px-[10px]">User</th>
                                ...
                                <tbody className="">
                                    {displayedData?.map((item, index) => (
                                        <tr
                                            ...
                                            onClick={() => navigate("/order_deatils", {
                                                state: {
                                                    orderId: item,
                                                },
                                            })}
                                        ...
                                        >
                                            <td className="text-sm text-[#5E5873] flex  px-[30px] max-xl:px-[10px] max-sm:px-[5px]   items-center gap-2 my-[15px] max-md:gap-1 max-md:text-[12px]">
                                                <div className="">
                                                    {item.order_id}
                                                </div>
                                            </td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px]  max-lg:hidden max-md:text-[12px]">{formatDate(item.created_at)}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:hidden max-md:text-[12px]">{item.price}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:hidden max-md:text-[12px]">{profile?.data?.username}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:text-[12px]">
                                                <button
                                                    className={`px-[16px] py-1 rounded-[400px] font-medium text-sm
    ...
    ${item?.status === 'Success' && 'bg-[#bbf7d0] text-[#166534]'}
  `}
                                                >
                                                    {item?.status}
                                                </button>
                                            ...
                                            {/* <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:text-[12px]">
                                                {item?.payment_status == 'Success' && (
                                                    <div>
                                                        ...
                                                        }} className='px-[16px] inline-flex lea items-center justify-center bg-primary h-[35px] border border-primary text-[white] text-[14px] rounded-[0.5rem]'>Invoice</a>
                                                    </div>
                                                )}
                                            ...
                                            {/* <td className="text-sm text-[#5E5873] text-center px-[30px] max-xl:px-[10px] max-sm:px-[5px]">
                                                <div className="flex gap-2 justify-center max-md:gap-1 max-sm:gap-0">
                                                    ...
                                                    <button className="w-[44px] h-[32px] max-md:h-[35px] max-md:w-[35px] bg-[#ff6c2f1a] flex justify-center items-center text-primary hover:bg-primary hover:text-white rounded-[0.5rem]">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="1.3em" height="1.3em" viewBox="0 0 24 24">
                        ...
                        {order?.total_items > itemsPerPage && (
