...
import { useDeleteProductMutation, useDeleteThemesMutation, useGetCategories_fetch_allQuery, useGetCategoriesQuery, useGetSubCategories_fetchallQuery, useGetSubCategoriesQuery, useGetThemesQuery } from "../../services/apiSlice";
...
const Product = () => {
    ...
    const { data: catee } = useGetCategories_fetch_allQuery()
    const { data: subcatee } = useGetSubCategories_fetchallQuery()

    const [searchTerm, setSearchTerm] = useState("");
    const [itemsPerPage, setItemsPerPage] = useState(10);
    const [currentPage, setCurrentPage] = useState(() => {
        return parseInt(sessionStorage.getItem("theme_page") || "1");
      });
    ...
    const [deletetheme] = useDeleteThemesMutation();
    const themedata = product?.data || [];
    ...
    const handleDeleteCategory = async () => {
        try {
            ...
            formdata.append('theme_id', deleteProduct)

            await deletetheme(formdata).unwrap();
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    <SubHeader setIsOpenside={setIsOpenside} pageName={"Themes"} />
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        <h3 className="text-[22px] font-[700] text-[#707793] uppercase ">Themes</h3>
                        ...
                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">
                                ...
                                <li className="text-primary font-medium text-[12px]">All Themes</li>
                            </ol>
                        </nav>
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        <div className="p-[20px] flex  gap-4 items-center justify-between">
                            ...
                            <div>
                                <h3 className="font-[600] text-[16px] text-[#313b5e] hans">All Themes List</h3>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <Link to="/create_theme">
                                        ...
                                        <button className="bg-[#ff6c2f] text-white text-[12.6px] rounded-[8px] py-[5.56px] px-[12px]">Add Themes</button>
                                    </Link>
                                </div>
                                <div>
                                    ...
                                    <div className="relative inline-block text-left max-sm:hidden" ref={dropdownRef}>
                                        <button
                                            onClick={() => navigate('/bluk_import_theme')}
                        ...
                        <div className="min-h-[65vh] over__scroll">
                            <table className="w-full ">
                                <thead className="border-y border-[#ddd]">
                                    ...
                                    <tr className="max-sm:h-[40px] h-[54px] ">
                                        ...
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap    max-md:text-[13px] max-sm:px-[10px]">Dis Price</th>
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap    max-md:text-[13px] max-sm:px-[10px]">Price</th>
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-sm:hidden  max-md:text-[13px] max-sm:px-[10px]">Category</th>
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-md:hidden  max-md:text-[13px] max-sm:px-[10px]">Sub Category</th>
                                        ...
                                        <tr
                                            ...
                                            onClick={(e) => {
                                                ...
                                                if (window.innerWidth <= 640) {
                                                    sessionStorage.setItem("theme_page", currentPage);
                                                    navigate("/edit_theme", {
                                                        state: { product_id: item?.theme_id }, // Passing product_id to state
                                                    })
                                                }
                                            }}
                                        >
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px]  max-md:text-[12px] ">{item?.discount_price}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px]  max-md:text-[12px] ">{item?.price}</td>

                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-sm:hidden  max-md:text-[12px]">{catee?.data?.find((val) => val.category_id == item?.category)?.name}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:hidden  max-md:text-[12px]">{subcatee?.data?.find((val) => val.subcategory_id == item?.subcategory)?.name}</td>
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px] max-xl:px-[10px] max-sm:px-[5px]">
                                                <div className="flex gap-2 justify-center max-md:gap-1 max-sm:gap-0">
                                                    ...
                                                    <button
                                                        className="w-[44px] h-[32px] max-sm:hidden max-md:h-[35px] max-md:w-[35px] me-2 bg-[#ff6c2f1a] flex justify-center items-center text-primary rounded-[0.5rem] hover:bg-primary hover:text-white"
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            sessionStorage.setItem("theme_page",currentPage)
                                                            navigate("/edit_theme", {
                                                    ...
                                                    <button className="w-[44px] h-[32px] me-2 max-md:h-[35px] max-md:w-[35px] bg-[#ff6c2f1a] flex justify-center items-center text-primary hover:bg-primary hover:text-white rounded-[0.5rem]" onClick={(e) => {
                                                        e.stopPropagation();
                                                        openModal('modal3')
                                                        setDeletProduct(item?.theme_id);
                                                    }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="1.3em" height="1.3em" viewBox="0 0 24 24">
